...
/**
 * ชื่อและที่อยู่หน่วยงานไม่ได้อยู่ในไฟล์คำแปล
 *
 * lib/organization-identity.ts อ่านค่าจาก NEXT_PUBLIC_ORG_* เพื่อให้ลูกค้าที่นำ
 * GACP Lite ไปใช้เปลี่ยนเป็นชื่อหน่วยงานของตัวเองได้ · การพิมพ์ชื่อซ้ำไว้ที่นี่ทำให้
 * มีแหล่งความจริงสองที่ แล้ว footer กับหน้า verify ก็จะพูดคนละชื่อกันเมื่อมีคนตั้ง env
 *
 * คำแปลอื่นในไฟล์นี้ยังเป็นข้อความคงที่ตามเดิม — ที่ย้ายมาคือเฉพาะสองค่าที่เป็น
 * "ตัวตนของหน่วยงาน" ไม่ใช่ "ภาษาไทยของข้อความ"
 */
export const thCore = {
common: {
        ministryName: ORGANIZATION.name,
        ministryAddress: ORGANIZATION.address,
...
renewalAdvisory: {
        ...
        body: `ฟีเจอร์ชำระเงินค่าต่ออายุยังอยู่ระหว่างพัฒนา กรุณาติดต่อ${ORGANIZATION.name} เพื่อดำเนินการชำระเงินและออกใบรับรองฉบับใหม่ โดยส่งอีเมลถึง ${ORGANIZATION.email} หรือผ่านหน้าติดต่อเจ้าหน้าที่`,
